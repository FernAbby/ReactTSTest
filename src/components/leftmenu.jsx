...
class LinkGroup extends Component {
    ...
    render() {
        let {item_title, id, item_link, item_icon, item_icon_blue, link_id, selectedLink,item_target} = {...this.props};
        let classStr = (id === link_id ? 'leftLink selected' : 'leftLink');
        let linkElement = '';
        if(parseInt(item_target,10)===1 ){
            linkElement = (
                <a className="leftLink" target="_blank" href={item_link} style={{cursor:'pointer'}}>
...
class Leftmenu extends Component {
    ...
    componentDidMount() {
        ...
        Requests.GetMenuList().then((data) => {
            if (data.code === 10000) {
